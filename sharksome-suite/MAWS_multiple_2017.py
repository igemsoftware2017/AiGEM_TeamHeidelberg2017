...
for ntide in 'GAUC':
	...
	#Check if best ...
	if len(best_entropies) < NKEEP:
		best_entropies.append(entropy)
		best_sequences.append(ntide)
		...
		best_sequences[worst_entropy_index] = ntide
...
for idx in range(NKEEP):
	...
	#Output best as well
	pdblog = open("{0}_best_cand{2}_1_{1}.pdb".format(JOB_NAME,ntide,idx),"w")
	app.PDBFile.writeModel(best_topologies[idx], best_positions[idx], file=pdblog, modelIndex=1)
	pdblog.close()

	output.write("{0}: Completed first step. Selected nucleotide: {1}\n".format(str(datetime.now()), best_sequences[idx]))
...
for i in range(N_NTIDES):
	best_old_sequences = best_sequences[:]
	best_old_positions = copy.deepcopy(best_positions)
	best_entropies = []
	best_sequences = []
	best_positions = []
	best_topologies = []
	for candidate in range(NKEEP):
		#Same as above, more or less
		best_old_sequence = best_old_sequences[candidate]
		best_old_position = best_old_positions[candidate][:]
		for ntide in 'GAUC':
	...
	for idx in range(NKEEP):
		app.PDBFile.writeModel(best_topologies[idx], best_positions[idx], file=step, modelIndex=1)
		#Output best as well
		output.write("{0}: Completed step {1}. Selected sequence: {2}\n".format(str(datetime.now()), i+2, best_sequences[idx]))
		pdblog = open("{0}_best_cand{3}_{1}_{2}.pdb".format(JOB_NAME, i+2, ntide, idx),"w")
		app.PDBFile.writeModel(best_topologies[idx], best_positions[idx], file=pdblog, modelIndex=1)
		pdblog.close()


for idx in range(NKEEP):
	best_sequence = best_sequences[idx]
	...
	result_complex.positions = best_positions[idx][:]
	pdb_result = open("{0}_RESULT_CANDIDATE_{1}.pdb".format(JOB_NAME, idx),"w")
