...
#COPYRIGHT 2016 Michael Jendrusch
...
from Routines import ZPS, S, ScoreThreshold
...
PDB_PATH = "/net/bq-lsdf/bioquant/ag-reils/igem2015/testenv/sharksome-suite/1gfl.pdb"
if args.path:
	PDB_PATH = args.path
#N_CHUNKS = 1
#FUZZY = 1
...
for ntide in ['ALA','GLY','GLU','HIS']:
	...
	#Build the Complex
	complex.build()
	currentCenter = centerOfMass(np.asarray(nostrom(complex.positions[complex.chains[1].start:])))
	shift = currentCenter*unit.angstrom - centerOfMass(np.asarray(nostrom(c.positions)))*unit.angstrom
	aptamer.translate_global(shift)
	complex.positions = complex.positions[:complex.chains[1].start] + c.positions[:]
	...
	#Calculate entropy
	entropy = ScoreThreshold(energies, BETA, 0)
		...
		for append in [True, False]:
			...
			for k in range(CHUNK_SIZE):
				...
				if append:
					...
					for j in range(N_ELEMENTS - 1):
						aptamer.rotate_in_residue(1, j, rotation[j], reverse=True)
				#For everything backward (C3'-O3')
				#Rotate the old nucleotides' bond
				if append:
					aptamer.rotate_in_residue(-2, 2, rotation[2])
				else:
					for j in range(N_ELEMENTS - 1, N_ELEMENTS):
